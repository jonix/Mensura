...
# ==============================
# Bash CLI Parser – körbar & sourcbar!
# ==============================
...
function parse-arg-example() {
    # ================== DEKLARATIONER, men ingen kod körs direkt! ==================
    FLAGS=(
...
print_help() {
...

  {
    ...
    for def in "${flag_defs[@]}"; do
      ...
      flag=$(trim_cmd_args "$flag"); short=$(trim_cmd_args "$short"); type=$(trim_cmd_args "$type")
      required=$(trim_cmd_args "$required"); default=$(trim_cmd_args "$default"); desc=$(trim_cmd_args "$desc")
      valtype=$(trim_cmd_args "$valtype"); regex=$(trim_cmd_args "$regex")
      local flagstr="--$flag"
    ...
    if [[ "${#pos_defs[@]}" -gt 0 ]]; then
      ...
      for pos in "${pos_defs[@]}"; do
        ...
        pname=$(trim_cmd_args "$pname"); ptype=$(trim_cmd_args "$ptype")
        pdesc=$(trim_cmd_args "$pdesc"); pdefault=$(trim_cmd_args "$pdefault")
...
parse_args() {
  ...
  # Bygg flaggtabeller med trim_cmd_argsning
  for def in "${flags[@]}"; do
    IFS='|' read -r flag short type required default desc valtype regex <<< "$def"
    flag=$(trim_cmd_args "$flag"); short=$(trim_cmd_args "$short"); type=$(trim_cmd_args "$type")
    required=$(trim_cmd_args "$required"); default=$(trim_cmd_args "$default"); desc=$(trim_cmd_args "$desc")
  ...
  # Positionella tabeller med trim_cmd_args
  ...
  while [[ $# -gt 0 ]]; do
    ...
    if [[ "$arg" =~ ^--([a-zA-Z0-9_-]+)=(.*)$ ]]; then
      key=$(trim_cmd_args "${BASH_REMATCH[1]//-/_}")
      val="${BASH_REMATCH[2]}"
      type="${FLAG_TYPES[$key]}"
      if [[ -z "$type" ]]; then echo -e "${CERR}Unknown option: --$key${CR}" >&2; return 1; fi
      if [[ "$type" == "h" ]]; then print_help "$0" flags positionals_spec; return 0
      ...
      key=$(trim_cmd_args "${BASH_REMATCH[1]//-/_}")
      type="${FLAG_TYPES[$key]}"
      if [[ -z "$type" ]]; then echo -e "${CERR}Unknown option: $arg${CR}" >&2; return 1; fi
      if [[ "$type" == "h" ]]; then print_help "$0" flags positionals_spec; return 0
      ...
      s=$(trim_cmd_args "${BASH_REMATCH[1]}")
      key="${FLAG_SHORTS[$s]}"
      key=$(trim_cmd_args "$key")
      type="${FLAG_TYPES[$key]}"
      if [[ -z "$key" ]]; then echo -e "${CERR}Unknown short option: $arg${CR}" >&2; return 1; fi
      if [[ "$type" == "h" ]]; then print_help "$0" flags positionals_spec; return 0
      elif [[ "$type" == "s" ]]; then FLAG_VALUES["$key"]="$2"; FLAG_SET["$key"]=1; shift 2
      elif [[ "$type" == "m" ]]; then FLAG_VALUES["$key"]+="${FLAG_VALUES[$key]:+|}$2"; FLAG_SET["$key"]=1; shift 2
      elif [[ "$type" == "b" ]]; then FLAG_VALUES["$key"]=1; FLAG_SET["$key"]=1; shift
  ...
  for def in "${flags[@]}"; do
    IFS='|' read -r flag short type required default desc valtype regex <<< "$def"
    flag=$(trim_cmd_args "$flag")
    required=$(trim_cmd_args "$required")
    default=$(trim_cmd_args "$default")
  ...
  for def in "${flags[@]}"; do
    IFS='|' read -r flag short type required default desc valtype regex <<< "$def"
    flag=$(trim_cmd_args "$flag"); valtype=$(trim_cmd_args "$valtype"); regex=$(trim_cmd_args "$regex")
    val="${FLAG_VALUES[$flag]}"
    val_trim_cmd_argsmed=$(trim_cmd_args "$val")
    required=$(trim_cmd_args "$required")
    if [[ "$required" == "1" && "$val_trim_cmd_argsmed" == "$SENTINEL" ]]; then
      echo -e "${CERR}Missing required argument: --$flag${CR}" >&2; errors=1
    fi
    if [[ -n "$val_trim_cmd_argsmed" && "$val_trim_cmd_argsmed" != "$SENTINEL" && -n "$valtype" ]]; then
      if ! check_valtype "$val_trim_cmd_argsmed" "$valtype" "${regex:-}"; then
  ...
  for idx in "${!positionals_spec[@]}"; do
    IFS=':' read -r pname ptype pdesc pdefault <<< "${positionals_spec[$idx]}"
    pname=$(trim_cmd_args "$pname"); ptype=$(trim_cmd_args "$ptype"); pdefault=$(trim_cmd_args "$pdefault")
    ...
    pval_trim_cmd_argsmed=$(trim_cmd_args "$pval")
    if [[ -n "$pval_trim_cmd_argsmed" && -n "$ptype" ]]; then
      if ! check_valtype "$pval_trim_cmd_argsmed" "$ptype"; then
...
# ================== ENDAST när scriptet körs direkt, inte när det sourcas! ==================
